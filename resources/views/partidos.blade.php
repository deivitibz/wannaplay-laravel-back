                ...
                <table class="table table-dark table-striped table-hover d-inline-table">
                    <tbody>
                      ...
                      <tr>
                          <th scope="row">
                            <span class="custom-checkbox">
                                <input type="checkbox" id="{{$partido->id}}" name="options[]" value="{{$partido->id}}">
                                <label for="checkbox1"></label>
                            </span>
                          </th>
                          <td>{{ucfirst($partido->deporte)}}</td>
                          <td>{{$jugadores->where('id', $partido->id_jugador_A1)->first()->nombre . " " . $jugadores->where('id', $partido->id_jugador_A1)->first()->apellido}}</td>
                          <td>{{$jugadores->where('id', $partido->id_jugador_A2)->first()->nombre . " " . $jugadores->where('id', $partido->id_jugador_A2)->first()->apellido}}</td>
                          <td>{{$jugadores->where('id', $partido->id_jugador_B1)->first()->nombre . " " . $jugadores->where('id', $partido->id_jugador_A1)->first()->apellido}}</td>
                          <td>{{$jugadores->where('id', $partido->id_jugador_B2)->first()->nombre . " " . $jugadores->where('id', $partido->id_jugador_A2)->first()->apellido}}</td>
                          <td>{{$partido->fecha}}</td>
                          <td>{{$partido->hora}}</td>
